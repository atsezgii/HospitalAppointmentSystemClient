 ...
 *     06.  Active Sidebar Menu  *
 ...
 function activateSidebarMenu() {
   ...
   if (current !== "" && document.getElementById("sidebar")) {
       var menuItems = document.querySelectorAll('#sidebar a, #sidebar [routerLink]');
       for (var i = 0, len = menuItems.length; i < len; i++) {
           var href = menuItems[i].getAttribute("href") || menuItems[i].getAttribute("routerLink");
 ...
 if(document.getElementById("sidebar")){
     ...
     for(var i = 0, len = elements.length; i < len; i++) {
         elements[i].onclick = function (elem) {
             var activeDropdown = document.querySelector("li.sidebar-dropdown.active");
             var activeSubmenu = document.querySelector("div.sidebar-submenu.d-block");

             // Mevcut açık menüyü kapat
             if(activeDropdown && activeDropdown !== elem.target.parentElement){
                 activeDropdown.classList.remove("active");
             }
             if(activeSubmenu && activeSubmenu !== elem.target.nextElementSibling){
                 activeSubmenu.classList.remove("d-block");
             }
